...
class axis_2(QWidget):
    def __init__(self, parent=None):
        super(axis_2, self).__init__(parent=parent)
        self.title = 'Axis Camera 2'
...
class Thread(QThread):
    ...
    def run(self):
        cap = cv2.VideoCapture('http://root:mrm@192.168.1.90/axis-cgi/mjpg/video.cgi?camera=2')#'rtsp://192.168.1.8/user=admin&password=&channel=1&stream=0.sdp?real_stream--rtp-caching=1')
